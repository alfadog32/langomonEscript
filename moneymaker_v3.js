...
let WebSocketImpl = null;try {// Prefer the ws package over Node's built-in WebSocket so close/error// events are easier to inspect under PM2.WebSocketImpl = require('ws');} catch {WebSocketImpl = globalThis.WebSocket || null;}
...
const CONFIG = {gammaBaseUrl: envStr('GAMMA_BASE_URL', 'https://gamma-api.polymarket.com'),clobBaseUrl: envStr('CLOB_BASE_URL', 'https://clob.polymarket.com'),clobWsUrl: envStr('CLOB_WS_URL', 'wss://ws-subscriptions-clob.polymarket.com/ws/market'),

// WebSocket is useful for speed, but REST polling is more reliable while// debugging. Turn it on from .env with ENABLE_WS=true after WS is stable.enableWs: envBool('ENABLE_WS', false),wsHeartbeatEnabled: envBool('WS_HEARTBEAT_ENABLED', true),wsHeartbeatMs: envInt('WS_HEARTBEAT_MS', 10_000),wsReconnectInitialMs: envInt('WS_RECONNECT_INITIAL_MS', 5_000),wsReconnectMaxMs: envInt('WS_RECONNECT_MAX_MS', 60_000),saveState: envBool('SAVE_STATE', true),stateFile: envStr('STATE_FILE', path.join(process.cwd(), 'moneymaker_v3_state.json')),

initialCash: envNum('INITIAL_CASH', 250),

eventLimit: envInt('EVENT_LIMIT', 100),eventPages: envInt('EVENT_PAGES', 2),maxMarkets: envInt('MAX_MARKETS', 20),maxOutcomesPerMarket: envInt('MAX_OUTCOMES_PER_MARKET', 2),marketRefreshEveryCycles: envInt('REFRESH_RESEARCH_EVERY', 10),

minLiquidity: envNum('MIN_LIQUIDITY', 500),minVolume24h: envNum('MIN_VOLUME_24H', 50),minBestBid: envNum('MIN_BEST_BID', 0.02),maxBestAsk: envNum('MAX_BEST_ASK', 0.98),maxSpread: envNum('MAX_SPREAD', 0.18),

hunterMode: envBool('HUNTER_MODE', true),hunterMaxSpread: envNum('HUNTER_MAX_SPREAD', 0.22),hunterMinTopDepthUsd: envNum('HUNTER_MIN_TOP_DEPTH_USD', 5),hunterMaxTopDepthUsd: envNum('HUNTER_MAX_TOP_DEPTH_USD', 4_000),

baseOrderUsd: envNum('BASE_ORDER_USD', 25),minOrderUsd: envNum('MIN_ORDER_USD', 3),maxPositionUsdPerAsset: envNum('MAX_POSITION_USD', 200),maxMarketExposureUsd: envNum('MAX_MARKET_EXPOSURE_USD', 350),maxTotalExposureUsd: envNum('MAX_TOTAL_EXPOSURE_USD', 1_500),maxTotalOpenOrderUsd: envNum('MAX_TOTAL_OPEN_ORDER_USD', 1_000),maxOpenOrders: envInt('MAX_OPEN_ORDERS', 250),maxDrawdownPct: envNum('MAX_DRAWDOWN_PCT', 12),

// Practical revenue/risk optimization controls.// These keep the paper engine realistic: no instant fantasy fills, no unlimited bags.stopLossPct: envNum('STOP_LOSS_PCT', 8),takeProfitPct: envNum('TAKE_PROFIT_PCT', 18),enableTakeProfit: envBool('ENABLE_TAKE_PROFIT', true),maxAdverseMovePct: envNum('MAX_ADVERSE_MOVE_PCT', 4),partialFillDepthFraction: envNum('PARTIAL_FILL_DEPTH_FRACTION', 0.35),minFillUsd: envNum('MIN_FILL_USD', 1),liquidityConsumedLimitPct: envNum('LIQUIDITY_CONSUMED_LIMIT_PCT', 0.20),liquidityDecayPower: envNum('LIQUIDITY_DECAY_POWER', 6.0),

// Ghost mode records would-be orders and checks where the midpoint moved later.// This helps calibrate quote offsets without pretending every order fills.enableGhostMode: envBool('ENABLE_GHOST_MODE', true),ghostHorizonMs: envInt('GHOST_HORIZON_MS', 60_000),ghostMaxRecords: envInt('GHOST_MAX_RECORDS', 500),

// 1) Order-book imbalance signals.enableImbalanceSignals: envBool('ENABLE_IMBALANCE_SIGNALS', true),imbalanceDepthLevels: envInt('IMBALANCE_DEPTH_LEVELS', 3),imbalanceStrongThreshold: envNum('IMBALANCE_STRONG_THRESHOLD', 0.25),imbalanceBalancedThreshold: envNum('IMBALANCE_BALANCED_THRESHOLD', 0.12),

// 2) Adaptive position sizing.enableAdaptiveSizing: envBool('ENABLE_ADAPTIVE_SIZING', true),adaptiveMinSizeMultiplier: envNum('ADAPTIVE_MIN_SIZE_MULTIPLIER', 0.35),adaptiveMaxSizeMultiplier: envNum('ADAPTIVE_MAX_SIZE_MULTIPLIER', 1.35),adaptiveGhostPenalty: envNum('ADAPTIVE_GHOST_PENALTY', 0.65),

// 3) Whale tracking hook. This reads public/externally collected whale events// from a local JSON file if you wire one in. It never invents whale data.enableWhaleTracking: envBool('ENABLE_WHALE_TRACKING', true),whaleEventsFile: envStr('WHALE_EVENTS_FILE', path.join(process.cwd(), 'whale_events.json')),whaleLookbackMs: envInt('WHALE_LOOKBACK_MS', 120_000),whaleMinUsd: envNum('WHALE_MIN_USD', 5_000),whaleAlignmentBoost: envNum('WHALE_ALIGNMENT_BOOST', 0.12),whaleDataApiUrl: envStr('WHALE_DATA_API_URL', 'https://data-api.polymarket.com'),whaleWallets: envList('WHALE_WALLETS', []),whalePollMs: envInt('WHALE_POLL_MS', 30_000),whaleApiTimeoutMs: envInt('WHALE_API_TIMEOUT_MS', 8_000),whaleTradesLimit: envInt('WHALE_TRADES_LIMIT', 50),whaleBatchSize: envInt('WHALE_BATCH_SIZE', 3),whaleBatchDelayMs: envInt('WHALE_BATCH_DELAY_MS', 1_000),enableWhaleCopyStrategy: envBool('ENABLE_WHALE_COPY_STRATEGY', true),whaleCopyFreshMs: envInt('WHALE_COPY_FRESH_MS', 30_000),whaleCopyBaseMultiplier: envNum('WHALE_COPY_BASE_MULTIPLIER', 0.4),whaleCopyWhaleFraction: envNum('WHALE_COPY_WHALE_FRACTION', 0.15),

// Multi-view consensus gate. This is the reports.js idea rebuilt with real// MoneyMaker data instead of random fake scouts or a second wallet engine.enableConsensus: envBool('ENABLE_CONSENSUS', true),consensusThreshold: envNum('CONSENSUS_THRESHOLD', 0.68),consensusLogRejected: envBool('CONSENSUS_LOG_REJECTED', false),consensusBoostMax: envNum('CONSENSUS_BOOST_MAX', 1.15),consensusPenaltyMin: envNum('CONSENSUS_PENALTY_MIN', 0.70),consensusStableMaxSpread: envNum('CONSENSUS_STABLE_MAX_SPREAD', 0.12),consensusTrendMovePct: envNum('CONSENSUS_TREND_MOVE_PCT', 0.035),consensusSniperSizeMultiplier: envNum('CONSENSUS_SNIPER_SIZE_MULTIPLIER', 0.65),consensusMakerBoost: envNum('CONSENSUS_MAKER_BOOST', 1.05),targetWalletHandle: envStr('TARGET_WALLET_HANDLE', 'gabagool22'),targetWalletMode: envBool('TARGET_WALLET_MODE', true),targetWalletDisplacementPct: envNum('TARGET_WALLET_DISPLACEMENT_PCT', 0.015),makerSpreadMultiplier: envNum('MAKER_SPREAD_MULTIPLIER', 1.2),

minSignalEdge: envNum('MIN_SIGNAL_EDGE', 0.008),minConfidence: envNum('MIN_CONFIDENCE', 0.45),slippageBuffer: envNum('SLIPPAGE_BUFFER', 0.004),adverseSelectionBuffer: envNum('ADVERSE_SELECTION_BUFFER', 0.006),

quoteEdgeTicks: envInt('QUOTE_EDGE_TICKS', 1),orderTtlMs: envInt('ORDER_TTL_MS', 45_000),maxHoldMs: envInt('MAX_HOLD_MS', 20 * 60_000),

loopDelayMs: envInt('LOOP_DELAY_MS', 6_000),wsDebounceMs: envInt('WS_DEBOUNCE_MS', 250),reportEveryCycles: envInt('REPORT_EVERY_CYCLES', 3),

historyLookback: envInt('HISTORY_LOOKBACK', 30),volatilityTripPct: envNum('VOL_TRIP_PCT', 12),// Binary markets can move violently. Keep cooldown short, but make edge demands// stricter while volatility is elevated.volatilityCooldownMs: envInt('VOL_COOLDOWN_MS', 5_000),volatilityEdgeMultiplier: envNum('VOL_EDGE_MULTIPLIER', 1.75),quoteDuringVolatility: envBool('QUOTE_DURING_VOL', false),

complementArbEnabled: envBool('STRAT_COMPLEMENT_ARB', true),spreadHunterEnabled: envBool('STRAT_SPREAD_HUNTER', true),inventoryExitEnabled: envBool('STRAT_INVENTORY_EXIT', true),tailEndEnabled: envBool('STRAT_TAIL_END', true),

complementArbMinEdge: envNum('COMPLEMENT_ARB_MIN_EDGE', 0.012),spreadHunterMinEdge: envNum('SPREAD_HUNTER_MIN_EDGE', 0.01),tailEndHours: envNum('TAIL_END_HOURS', 36),tailEndMinConfidence: envNum('TAIL_END_MIN_CONFIDENCE', 0.58),};

// =========================// ENV HELPERS// =========================

function loadDotEnvFile(filePath = path.join(process.cwd(), '.env')) {try {if (!fs.existsSync(filePath)) return;
...
} catch (e) {console.warn([ENV] Failed to load .env file: ${e.message});}}

function envStr(name, fallback) {return process.env[name] ?? fallback;}

function envNum(name, fallback) {const raw = process.env[name];const n = Number(raw);return Number.isFinite(n) ? n : fallback;}

function envInt(name, fallback) {const raw = process.env[name];const n = Number.parseInt(raw, 10);return Number.isFinite(n) ? n : fallback;}

function envBool(name, fallback) {const raw = process.env[name];if (raw === undefined) return fallback;return ['1', 'true', 'yes', 'y', 'on'].includes(String(raw).toLowerCase());}

function envList(name, fallback = []) {const raw = process.env[name];if (!raw) return fallback;return String(raw).split(',').map((x) => x.trim()).filter(Boolean);}

// =========================// LOGGING// =========================

function log(level, message) {console.log(${new Date().toISOString()} [${level}] ${message});}

const info = (m) => log('INFO', m);const warn = (m) => log('WARN', m);const errlog = (m) => log('ERROR', m);

// =========================// HTTP CLIENT// =========================

class HttpClient {constructor({ timeoutMs = 12_000, retries = 2 } = {}) {this.timeoutMs = timeoutMs;this.retries = retries;}

async getJson(url) {let lastErr;
...
class PolymarketPublicClient {constructor(config) {this.config = config;this.http = new HttpClient();}

async fetchActiveEvents() {const all = [];
...
extractTradableMarkets(events) {const markets = [];
...
async getOrderBook(tokenId) {const url = new URL('/book', this.config.clobBaseUrl);url.searchParams.set('token_id', String(tokenId));
...
function isMarketTradable(market) {const active = market.active !== false;const closed = market.closed === true;const archived = market.archived === true;const enableOrderBook =market.enableOrderBook === true ||market.enable_order_book === true ||market.enableOrderBook === 'true' ||market.enable_order_book === 'true';

return active && !closed && !archived && enableOrderBook;}

function parseMaybeJsonArray(value) {if (Array.isArray(value)) return value;if (typeof value !== 'string') return null;

try {const parsed = JSON.parse(value);return Array.isArray(parsed) ? parsed : null;} catch {return value.split(',').map((s) => s.trim()).filter(Boolean);}};

function normalizeBook(raw, fallbackAssetId = '') {const bids = normalizeLevels(raw?.bids, 'bid');const asks = normalizeLevels(raw?.asks, 'ask');

bids.sort((a, b) => b.price - a.price);asks.sort((a, b) => a.price - b.price);

const bestBid = bids[0]?.price ?? toNum(raw?.best_bid ?? raw?.bestBid, NaN);const bestAsk = asks[0]?.price ?? toNum(raw?.best_ask ?? raw?.bestAsk, NaN);const safeBestBid = Number.isFinite(bestBid) ? bestBid : null;const safeBestAsk = Number.isFinite(bestAsk) ? bestAsk : null;const midpoint = safeBestBid !== null && safeBestAsk !== null ? (safeBestBid + safeBestAsk) / 2 : null;const spread = safeBestBid !== null && safeBestAsk !== null ? safeBestAsk - safeBestBid : null;

return {assetId: String(raw?.asset_id || raw?.assetId || fallbackAssetId || ''),market: String(raw?.market || ''),timestamp: String(raw?.timestamp || ''),bids,asks,bestBid: safeBestBid,bestAsk: safeBestAsk,midpoint,spread,minOrderSize: toNum(raw?.min_order_size ?? raw?.minOrderSize, 5),tickSize: toNum(raw?.tick_size ?? raw?.tickSize, 0.01),lastTradePrice: toNum(raw?.last_trade_price ?? raw?.lastTradePrice, NaN),cachedAt: Date.now(),};}

function normalizeLevels(levels, side) {if (!Array.isArray(levels)) return [];

return levels.map((x) => ({price: toNum(x.price, NaN),size: toNum(x.size, NaN),side,})).filter((x) => Number.isFinite(x.price) && Number.isFinite(x.size) && x.price > 0 && x.size > 0);}

// =========================// MARKET CACHE// =========================

class MarketCache {constructor(poly) {this.poly = poly;this.books = new Map();this.marketsById = new Map();this.assetsByToken = new Map();}

setCandidates(assets) {this.marketsById.clear();this.assetsByToken.clear();
...
getAsset(tokenId) {return this.assetsByToken.get(String(tokenId));}

getMarketAssets(marketId) {return [...this.assetsByToken.values()].filter((asset) => asset.market.marketId === marketId);}

setBook(tokenId, book) {if (!book) return;book.cachedAt = Date.now();this.books.set(String(tokenId), book);}

getBook(tokenId) {return this.books.get(String(tokenId));}

async getFreshBook(tokenId, maxAgeMs = 1_500) {const cached = this.getBook(tokenId);if (cached && Date.now() - (cached.cachedAt || 0) <= maxAgeMs && cached.midpoint !== null) {return cached;}
...
markPrices() {const map = new Map();for (const [tokenId, book] of this.books.entries()) {if (Number.isFinite(book.midpoint)) {map.set(tokenId, book.midpoint);}}return map;}}

// =========================// PUBLIC CLOB WEBSOCKET// =========================

class CLOBWebSocketClient {constructor({ url, onMessage, config = CONFIG }) {this.url = url;this.onMessage = onMessage;this.config = config;this.ws = null;this.assetIds = new Set();this.connected = false;this.reconnectTimer = null;this.pingTimer = null;this.reconnectDelayMs = config.wsReconnectInitialMs || 5_000;}

connect() {if (!WebSocketImpl) {warn('WebSocket disabled: install optional dependency with npm install ws.');return;}
...
handleOpen() {if (this.connected) return;this.connected = true;this.reconnectDelayMs = this.config.wsReconnectInitialMs || 5_000;info('CLOB WebSocket connected.');this.resubscribe();this.startPing();}

handleClose(code, reason) {this.connected = false;this.stopPing();clearTimeout(this.reconnectTimer);
...
startPing() {this.stopPing();if (!this.config.wsHeartbeatEnabled) return;
...
stopPing() {clearInterval(this.pingTimer);this.pingTimer = null;}

subscribe(assetIds) {const ids = Array.isArray(assetIds) ? assetIds : [assetIds];let changed = false;
...
resubscribe() {if (!this.ws || this.ws.readyState !== WebSocketImpl.OPEN) return;const ids = [...this.assetIds];if (ids.length === 0) return;
...
handleRawMessage(raw) {const text = Buffer.isBuffer(raw) ? raw.toString('utf8') : String(raw);if (!text || text === 'PONG') return;
...
class ResearchEngine {constructor(poly, cache, config) {this.poly = poly;this.cache = cache;this.config = config;}

async discoverCandidates() {info('Research refresh: fetching active events and books...');
...
scoreAsset(market, outcome, book) {if (!isBookComplete(book)) return null;if (book.bestBid < this.config.minBestBid) return null;if (book.bestAsk > this.config.maxBestAsk) return null;
...
function isBookComplete(book) {return Boolean(book &&Number.isFinite(book.bestBid) &&Number.isFinite(book.bestAsk) &&Number.isFinite(book.midpoint) &&Number.isFinite(book.spread) &&book.bestBid > 0 &&book.bestAsk > 0 &&book.bestBid < book.bestAsk);}

function topDepthUsd(levels, n) {return (levels || []).slice(0, n).reduce((sum, level) => sum + level.price * level.size, 0);}

function estimateLiquidityConsumption(book, side, sizeUsd, config = CONFIG) {if (!book || !Number.isFinite(sizeUsd) || sizeUsd <= 0) {return { consumedPct: 1, penalty: 0, topDepthUsd: 0 };}

// For maker buys, queue/competition is best approximated by bid-side depth.// For maker sells, queue/competition is best approximated by ask-side depth.const depthUsd = side === 'buy'? topDepthUsd(book.bids, 3): topDepthUsd(book.asks, 3);

const consumedPct = depthUsd > 0 ? sizeUsd / depthUsd : 1;const limit = Math.max(0.01, config.liquidityConsumedLimitPct || 0.20);

// Above the limit, assume fill probability decays exponentially instead of// pretending the whole order gets equal queue priority.const excess = Math.max(0, consumedPct - limit);const penalty = excess <= 0? 1: Math.exp(-excess * (config.liquidityDecayPower || 6.0));

return {consumedPct,penalty: clamp(penalty, 0.05, 1),topDepthUsd: depthUsd,};}

function computeOrderBookImbalance(book, levels = CONFIG.imbalanceDepthLevels) {if (!book || !Array.isArray(book.bids) || !Array.isArray(book.asks)) {return { bidDepthUsd: 0, askDepthUsd: 0, imbalance: 0, direction: 'unknown', usable: false };}

const bidDepthUsd = topDepthUsd(book.bids, levels);const askDepthUsd = topDepthUsd(book.asks, levels);const total = bidDepthUsd + askDepthUsd;const imbalance = total > 0 ? (bidDepthUsd - askDepthUsd) / total : 0;

let direction = 'balanced';if (imbalance >= CONFIG.imbalanceStrongThreshold) direction = 'bid_heavy';if (imbalance <= -CONFIG.imbalanceStrongThreshold) direction = 'ask_heavy';

return {bidDepthUsd,askDepthUsd,imbalance,direction,usable: total > 0,};}

function priceExtremePenalty(mid) {if (!Number.isFinite(mid)) return 100;if (mid > 0.12 && mid < 0.88) return 0;if (mid > 0.05 && mid < 0.95) return 8;return 25;}

function endingSoonPenalty(endDate) {const ms = msUntil(endDate);if (!Number.isFinite(ms)) return 0;if (ms < 2 * 60 * 60 * 1000) return 40;if (ms < 8 * 60 * 60 * 1000) return 15;return 0;}

// =========================// STRATEGY SIGNALS// =========================

class Signal {constructor({strategy,tokenId,marketId,side,price,sizeUsd,expectedEdge,confidence,reason,exitPlan,ttlMs,maxHoldMs,metadata = {},}) {this.id = crypto.randomUUID();this.strategy = strategy;this.tokenId = String(tokenId);this.marketId = String(marketId || '');this.side = side;this.price = price;this.sizeUsd = sizeUsd;this.expectedEdge = expectedEdge;this.confidence = confidence;this.reason = reason;this.exitPlan = exitPlan;this.ttlMs = ttlMs;this.maxHoldMs = maxHoldMs;this.metadata = metadata;this.createdAt = Date.now();}}

class Strategy {constructor(name, config, cache, portfolio, volGuard) {this.name = name;this.config = config;this.cache = cache;this.portfolio = portfolio;this.volGuard = volGuard;}

async generate() {return [];}}

class SpreadHunterStrategy extends Strategy {constructor(...args) {super('SpreadHunter', ...args);}

async generate(asset, book) {if (!this.config.spreadHunterEnabled) return [];if (!isBookComplete(book)) return [];
...
class InventoryExitStrategy extends Strategy {constructor(...args) {super('InventoryExit', ...args);}

async generate(asset, book) {if (!this.config.inventoryExitEnabled) return [];if (!isBookComplete(book)) return [];
...
class ComplementArbStrategy extends Strategy {constructor(...args) {super('ComplementArb', ...args);}

async generate(asset, book) {if (!this.config.complementArbEnabled) return [];if (!isBookComplete(book)) return [];
...
class TailEndMispricingStrategy extends Strategy {constructor(...args) {super('TailEndMispricing', ...args);}

async generate(asset, book) {if (!this.config.tailEndEnabled) return [];if (!isBookComplete(book)) return [];
...
class WhaleCopyStrategy extends Strategy {constructor(config, cache, portfolio, volGuard, whaleWatcher) {super('WhaleCopy', config, cache, portfolio, volGuard);this.whaleWatcher = whaleWatcher;}

async generate(asset, book) {if (!this.config.enableWhaleTracking || !this.config.enableWhaleCopyStrategy || !this.whaleWatcher) return [];if (!isBookComplete(book)) return [];
...
function confidenceFromPrice(mid) {return Math.abs(mid - 0.5) * 1.6;}

// =========================// WHALE WATCHER (ASYNC)// =========================

class AsyncWhaleWatcher {constructor(config) {this.config = config;this.baseUrl = config.whaleDataApiUrl;this.wallets = config.whaleWallets || [];this.whaleState = new Map();this.events = [];this.lastPollMs = 0;this.inFlight = false;
...
tick() {if (!this.config.enableWhaleTracking || this.wallets.length === 0) return;
...
async updateWhaleIntel(wallets, minUsd = this.config.whaleMinUsd) {if (!Array.isArray(wallets) || wallets.length === 0) return;if (this.inFlight) return;
...
async fetchWalletTrades(wallet, minUsd) {const controller = new AbortController();const timeout = setTimeout(() => controller.abort(), this.config.whaleApiTimeoutMs);
...
normalizeTrade(wallet, trade) {if (!trade) return null;
...
storeTrade(wallet, trade) {const key = ${wallet}:${trade.tokenId || trade.marketId || trade.marketTitle}:${trade.timestamp}:${trade.side}:${trade.sizeUsd};if (this.events.some((e) => e.key === key)) return;
...
prune() {const now = Date.now();this.events = this.events.filter((event) => now - event.timestamp <= this.config.whaleLookbackMs).sort((a, b) => b.timestamp - a.timestamp).slice(0, 500);}

isWhaleActiveOnMarket(marketTitle) {const now = Date.now();const title = normalizeTitle(marketTitle);const lookbackMs = this.config.whaleLookbackMs;
...
findRecentForSignal(signal) {if (!this.config.enableWhaleTracking || !signal) return null;this.prune();const now = Date.now();
...
function formatWsError(e) {if (!e) return 'unknown websocket error';if (typeof e === 'string') return e;if (e.message) return e.message;if (e.code) return String(e.code);if (e.type) return String(e.type);

try {return JSON.stringify(e);} catch {return String(e);}}

function normalizeTitle(value) {const cleaned = String(value || '').trim().toLowerCase().replace(/[^a-z0-9 ]/g, ' ');

return cleaned.split(' ').filter(Boolean).join(' ');}

// =========================// MULTI-VIEW CONSENSUS ENGINE// =========================

class MultiConsensusEngine {constructor(config) {this.config = config;this.midHistory = new Map();}

evaluateSignal(signal, asset, book, cache, portfolio, volGuard, whaleTracker = null) {if (!signal || !asset || !book) return null;
...
routeExecution(marketData) {const targetDisplacement = this.calculateTargetWalletDisplacement(marketData);const volatilityState = this.calculateVolatility(marketData, targetDisplacement);
...
calculateVolatility({ signal, book, volGuard }, targetDisplacement = null) {if (!signal || !isBookComplete(book)) return 'WAIT';if (volGuard?.isTripped?.(signal.tokenId) && !this.config.quoteDuringVolatility) return 'WAIT';
...
executeMakerStrategy({ signal, book, components, targetDisplacement, forcedReason }) {const makerStrategies = new Set(['SpreadHunter', 'ComplementArb']);const authorized = makerStrategies.has(signal.strategy) && components.depth >= 0.45 && components.volatility >= 0.36;
...
executeSniperStrategy({ signal, book, components }) {const arr = this.midHistory.get(String(signal.tokenId)) || [];const first = arr[0]?.mid;const last = arr[arr.length - 1]?.mid;const directionalMove = Number.isFinite(first) && Number.isFinite(last) ? last - first : 0;
...
applyExecutionRoute(signal, route) {if (!signal || !route) return signal;signal.confidence = clamp(signal.confidence * (route.confidenceMultiplier ?? 1), 0, 0.99);signal.expectedEdge = signal.expectedEdge * (route.edgeMultiplier ?? 1);
...
applyWhaleConsensusAdjustment(signal, whaleEvent) {if (!this.config.enableWhaleTracking || !signal || !whaleEvent) return signal;
...
applyAdaptivePositionSizing(signal, components, route, book, portfolio) {if (!this.config.enableAdaptiveSizing || !signal || signal.side !== 'buy') return signal;
...
calculateTargetWalletDisplacement({ signal, book, whaleEvent }) {if (!this.config.targetWalletMode || !signal || !isBookComplete(book)) {return { detected: false, reason: 'target wallet mode disabled or incomplete book' };}
...
scoreWhale(signal, whaleEvent) {if (!this.config.enableWhaleTracking) return 0.5;if (!whaleEvent) return 0.5;
...
recordMid(tokenId, mid) {if (!Number.isFinite(mid)) return;
...
scoreStructure(signal, asset, book, cache) {if (signal.strategy === 'ComplementArb') {const siblings = cache.getMarketAssets(asset.market.marketId);if (siblings.length < 2) return 0.25;
...
scoreDepth(book) {if (!isBookComplete(book)) return 0;const bidDepth = topDepthUsd(book.bids, this.config.imbalanceDepthLevels);const askDepth = topDepthUsd(book.asks, this.config.imbalanceDepthLevels);const weaker = Math.min(bidDepth, askDepth);const stronger = Math.max(bidDepth, askDepth);const balance = stronger > 0 ? weaker / stronger : 0;
...
scoreImbalance(signal, book) {if (!this.config.enableImbalanceSignals) return 0.5;
...
scoreMomentum(signal, book) {const arr = this.midHistory.get(String(signal.tokenId)) || [];if (arr.length < 4) return 0.55;
...
scoreVolatility(signal, book, volGuard) {if (!isBookComplete(book)) return 0;if (volGuard?.isTripped?.(signal.tokenId) && !this.config.quoteDuringVolatility) return 0.25;if (book.spread > this.config.hunterMaxSpread) return 0.2;return clamp(1 - (book.spread / Math.max(this.config.hunterMaxSpread, 0.01)) * 0.45, 0, 1);}

scorePortfolio(signal, book, portfolio) {if (!portfolio || !isBookComplete(book)) return 0.5;
...
scoreTiming(signal, asset) {const ms = msUntil(asset.market.endDate);if (!Number.isFinite(ms)) return 0.55;if (ms < 30 * 60_000) return 0.2;if (ms < 2 * 60 * 60_000) return 0.45;return 0.65;}}

// =========================// VOLATILITY GUARD// =========================

class VolatilityGuard {constructor(config) {this.config = config;this.history = new Map();this.trippedUntil = new Map();}

update(tokenId, midpoint) {if (!Number.isFinite(midpoint)) return;
...
isTripped(tokenId) {const until = this.trippedUntil.get(String(tokenId)) || 0;return until > Date.now();}

getVolMultiplier(tokenId) {return this.isTripped(tokenId) ? 3.0 : 1.0;}}

// =========================// PAPER PORTFOLIO / ACCOUNTING// =========================

class PaperPortfolio {constructor(config) {this.config = config;this.cash = config.initialCash;this.startingCash = config.initialCash;this.peakEquity = config.initialCash;this.positions = new Map();this.costBasis = new Map();this.openOrders = new Map();this.closedPnl = 0;this.strategyPnl = new Map();this.fills = [];this.ghostOrders = [];this.ghostStats = { total: 0, favorable: 0, unfavorable: 0 };}

position(tokenId) {return this.positions.get(String(tokenId)) || 0;}

avgCost(tokenId) {return this.costBasis.get(String(tokenId)) || 0;}

positionUsd(tokenId, mark) {return this.position(tokenId) * (Number.isFinite(mark) ? mark : this.avgCost(tokenId));}

marketExposureUsd(marketId, fallbackMark = 0.5) {let total = 0;
...
totalExposureUsd(fallbackMark = 0.5) {let total = 0;for (const [tokenId, qty] of this.positions.entries()) {if (qty > 0) total += qty * fallbackMark;}for (const order of this.openOrders.values()) {total += order.remainingUsd();}return total;}

equity(markPrices = new Map()) {let value = this.cash;for (const [tokenId, qty] of this.positions.entries()) {if (qty <= 0) continue;const mark = markPrices.get(tokenId) ?? this.avgCost(tokenId) ?? 0;value += qty * mark;}
...
drawdownPct(markPrices = new Map()) {const eq = this.equity(markPrices);if (this.peakEquity <= 0) return 0;return Math.max(0, ((this.peakEquity - eq) / this.peakEquity) * 100);}

getDrawdownPct(markPrices = new Map()) {return this.drawdownPct(markPrices);}

totalOpenOrderUsd() {let total = 0;for (const order of this.openOrders.values()) {total += order.remainingUsd();}return total;}

addOrder(order) {this.openOrders.set(order.id, order);}

cancelOrder(orderId) {this.openOrders.delete(orderId);}

recordGhostOrder(signal, book) {if (!this.config.enableGhostMode || !signal || !book || !Number.isFinite(book.midpoint)) return;
...
updateGhostOrders(markPrices = new Map()) {if (!this.config.enableGhostMode) return;
...
recordFill({ tokenId, marketId, side, price, size, strategy }) {tokenId = String(tokenId);marketId = String(marketId || '');
...
saveState() {if (!this.config.saveState) return;
...
loadState() {if (!this.config.saveState) return;
...
class PaperOrder {constructor(signal) {this.id = signal.id;this.signal = signal;this.tokenId = signal.tokenId;this.marketId = signal.marketId;this.side = signal.side;this.price = signal.price;this.sizeUsd = signal.sizeUsd;this.strategy = signal.strategy;this.createdAt = Date.now();this.expiresAt = Date.now() + signal.ttlMs;this.filledUsd = 0;this.metadata = signal.metadata || {};}

remainingUsd() {return Math.max(0, this.sizeUsd - this.filledUsd);}

isExpired() {return Date.now() >= this.expiresAt;}}

// =========================// RISK ENGINE// =========================

class RiskEngine {constructor(config, portfolio) {this.config = config;this.portfolio = portfolio;}

evaluate(signal) {if (!signal) return null;if (!['buy', 'sell'].includes(signal.side)) return null;if (!Number.isFinite(signal.price) || signal.price <= 0) return null;if (!Number.isFinite(signal.sizeUsd) || signal.sizeUsd <= 0) return null;
...
class PaperExecutionEngine {constructor(config, portfolio, cache) {this.config = config;this.portfolio = portfolio;this.cache = cache;}

place(signal, book) {const order = new PaperOrder(signal);this.portfolio.addOrder(order);this.portfolio.recordGhostOrder(signal, book);info([ORDER] ${signal.side.toUpperCase()} ${shortId(signal.tokenId)} @ ${fmtPrice(signal.price)} size=$${signal.sizeUsd.toFixed(2)} [${signal.strategy}]);}

processOpenOrders() {const now = Date.now();
...
computeFill(order, book) {if (!isBookComplete(book)) return null;
...
generateProtectiveSignals(asset, book) {if (!isBookComplete(book)) return [];
...
class BotEngine {constructor(config) {this.config = config;this.poly = new PolymarketPublicClient(config);this.cache = new MarketCache(this.poly);this.portfolio = new PaperPortfolio(config);this.portfolio.loadState();
...
async start() {banner();info('Starting Polymarket MoneyMaker V3 (Paper)...');
...
async refreshResearch() {this.assets = await this.research.discoverCandidates();
...
startWebSocket() {this.wsClient = new CLOBWebSocketClient({url: this.config.clobWsUrl,config: this.config,onMessage: (msg) => {const eventType = msg.event_type || msg.event || msg.type;const assetId = String(msg.asset_id || msg.assetId || msg.token_id || msg.tokenId || '');
...
async tick() {this.cycle += 1;this.whaleTracker?.tick?.();
...
trySignal(rawSignal, asset, book) {let signal = rawSignal;
...
report() {const markPrices = this.cache.markPrices();const equity = this.portfolio.equity(markPrices);const drawdown = this.portfolio.drawdownPct(markPrices);
...
function roundToTick(value, tick) {if (!Number.isFinite(value)) return value;if (!Number.isFinite(tick) || tick <= 0) return value;return Math.round(value / tick) * tick;}

function toNum(value, fallback = 0) {const n = Number(value);return Number.isFinite(n) ? n : fallback;}

function firstFinite(...values) {for (const value of values) {const n = Number(value);if (Number.isFinite(n)) return n;}return 0;}

function shortId(value) {const s = String(value || '');if (s.length <= 10) return s;return ${s.slice(0, 6)}...${s.slice(-4)};}

function fmtPrice(value) {if (!Number.isFinite(value)) return 'n/a';return $${Number(value).toFixed(3)};}

function chunks(arr, size) {const out = [];for (let i = 0; i < arr.length; i += size) {out.push(arr.slice(i, i + size));}return out;}

function sleep(ms) {return new Promise((resolve) => setTimeout(resolve, ms));}

function msUntil(dateLike) {if (!dateLike) return NaN;const t = Date.parse(dateLike);if (!Number.isFinite(t)) return NaN;return t - Date.now();}

function hoursUntil(dateLike) {const ms = msUntil(dateLike);if (!Number.isFinite(ms)) return NaN;return ms / (60 * 60 * 1000);}

function banner() {console.log('====================================================');console.log('  POLYMARKET MONEYMAKER V3.1.1 PAPER ENGINE');console.log('====================================================');}

// =========================// MAIN// =========================

async function main() {const bot = new BotEngine(CONFIG);await bot.start();}

main().catch((e) => {errlog(Fatal start error: ${e.stack || e.message});process.exitCode = 1;});
