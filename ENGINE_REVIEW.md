...
Bootstrap admission is now keeping the paper engine active with a small number of orders: recent metrics showed `Open Orders=2`, `Open Order Exposure=$6.45`, `paperOrdersPlacedLastHour=18`, `duplicateSkipsLastHour=9`, and `maxOpenOrderBlocksLastHour=0`. The remaining blocker is fillability: `0 fills / 18 paper orders = 0%` realized fill rate. Sophie was estimating fill probabilities around `0.33-0.36` for passive orders that were often `0.05-0.07` away from touch, which overstated fillability.

Predicted fill probability is now calibrated by quote distance. Orders at or inside `SOPHIE_FILL_DISTANCE_IDEAL` keep the raw estimate. Orders beyond that are gradually penalized until `SOPHIE_FILL_DISTANCE_MAX_REASONABLE`; farther orders are capped by `SOPHIE_FILL_PROB_CAP_WHEN_FAR`, and orders at or beyond `SOPHIE_FILL_DISTANCE_HARD_CAP` are capped by `SOPHIE_FILL_PROB_CAP_WHEN_VERY_FAR`. This makes passive far-from-touch orders fail fill-probability floors instead of consuming slots based on optimistic estimates.

Sophie also learns from admitted paper order outcomes. Orders that expire or are replaced without any fill are recorded as no-fill lifecycle events. Per token/side/strategy stats now track admitted, placed, filled, expired-without-fill, no-fill streak, realized fill rate, and time-to-fill. When a token/side accumulates a no-fill streak, predicted fill probability is multiplied down and new admissions can enter a cooldown. Bootstrap also has a same-token cooldown, so the same token cannot repeatedly cycle through bootstrap after no-fill outcomes unless quality or touch distance materially improves.

Paper maker nudge diagnostics can suggest a one-tick maker quote improvement after no-fill history, but `PAPER_MAKER_NUDGE_ENABLED` defaults to false. When disabled it only logs the suggested price and edge-after-nudge. If enabled for paper testing, it still cannot cross the spread and requires expected edge after the nudge to remain above `PAPER_MAKER_NUDGE_MIN_EDGE_AFTER_NUDGE`. This is safer than raising `MAX_OPEN_ORDERS` or lowering confidence because it narrows admission to more fillable paper orders without bypassing stale-book, volatility, cash, exposure, drawdown, duplicate, dust, ghost, or RiskEngine checks.
...
`npm run live:readiness` now keeps `READY_FOR_MICRO_LIVE=false` unless all safety and efficiency checks pass: expected PM2 processes online, safe live flags, recent portfolio report, `fillsLastHour >= 3`, `fillRateLastHour >= 1.0%`, `ordersPlacedLastHour <= 150`, `duplicateSkipsLastHour <= 500`, `maxOpenOrderBlocksLastHour <= 50`, `noFillStreakMax <= 3`, no excessive expired-without-fill outcomes, and no recent engine starvation warning. Passing readiness still means dry-run review only, not automatic live execution.
